...
epl_tables_list <- map(start_years, function(yr) {
  message("Downloading EPL table for season ", yr, "/", substr(yr + 1, 3, 4))
...
serieA_tables_list <- map(start_years, function(yr) {
  message("Downloading Serie A table for season ", yr, "/", substr(yr + 1, 3, 4))
...
laliga_tables_list <- map(start_years, function(yr) {
  message("Downloading LaLiga table for season ", yr, "/", substr(yr + 1, 3, 4))
...
bundesliga_tables_list <- map(start_years, function(yr) {
  message("Downloading Bundesliga table for season ", yr, "/", substr(yr + 1, 3, 4))
...
ligue1_tables_list <- map(start_years, function(yr) 
  message("Downloading Ligue 1 table for season ", yr, "/", substr(yr + 1, 3, 4))
